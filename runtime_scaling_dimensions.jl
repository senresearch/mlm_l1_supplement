...
Interactions are 1/8 nonzero drawn from Normal(0,2). Errors are drawn from 
Normal(0,3). 

"""

@everywhere function simRawData(n::Int64, m::Int64, p::Int64, q::Int64, 
    seed::Int64=10)
    # Set random seed
    srand(seed)
    ...
    inter = reshape(makeEffect((p)*(q), 1/8), p, q) 
...
Row and column main effects are 1/2 nonzero drawn from Normal(0,2). 
Interactions are 1/8 nonzero drawn from Normal(0,2). Errors are drawn from 
Normal(0,3). Default parameters are used for the call to mlmnet. 

"""

@everywhere function runSim(lambdas::Array{Float64,1}, 
                            fun::Function=fista_bt!; 
